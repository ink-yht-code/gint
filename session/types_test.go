...
func (m *mockSession) UserContext(ctx context.Context) (*gctx.UserContext, error) {
	uc := &gctx.UserContext{UserId: m.userId}
	if username, ok := m.data["username"]; ok {
		if v, ok := username.(string); ok {
			uc.Username = v
		}
	}
	if role, ok := m.data["role"]; ok {
